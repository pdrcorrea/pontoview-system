...
export async function encrypt(value: string) { const iv = crypto.getRandomValues(new Uint8Array(12)); const encrypted = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, await encryptionKey(), encoder.encode(value)); return `${bytesToBase64(iv)}.${bytesToBase64(new Uint8Array(encrypted))}`; }
export async function decrypt(value: string) { const [iv, payload] = value.split("."); const plain = await crypto.subtle.decrypt({ name: "AES-GCM", iv: base64ToBytes(iv) }, await encryptionKey(), base64ToBytes(payload)); return decoder.decode(plain); }

async function hmac(value: string) { const secret = Deno.env.get("OAUTH_STATE_SECRET"); if (!secret) throw new HttpError(503, "OAUTH_STATE_SECRET_MISSING"); const key = await crypto.subtle.importKey("raw", encoder.encode(secret), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]); return base64Url(new Uint8Array(await crypto.subtle.sign("HMAC", key, encoder.encode(value)))); }
export async function signState(payload: Record<string, unknown>) { const encoded = base64Url(encoder.encode(JSON.stringify(payload))); return `${encoded}.${await hmac(encoded)}`; }
export async function verifyState(value: string) { const [payload, signature] = value.split("."); if (!payload || !signature || signature !== await hmac(payload)) throw new HttpError(400, "INVALID_OAUTH_STATE"); const normalized = payload.replace(/-/g, "+").replace(/_/g, "/"); const json = decoder.decode(base64ToBytes(normalized.padEnd(Math.ceil(normalized.length / 4) * 4, "="))); const parsed = JSON.parse(json); if (!parsed.exp || parsed.exp < Date.now()) throw new HttpError(400, "EXPIRED_OAUTH_STATE"); return parsed as Record<string, unknown>; }
...
export async function getDriveAccessToken(connectionId: string) {
  const [{ data: connection }, { data: credentials }] = await Promise.all([
    ...
    admin.schema("private").from("drive_credentials").select("access_token_encrypted,refresh_token_encrypted,token_expires_at").eq("connection_id", connectionId).single(),
  ]);
  if (!connection || !credentials || connection.status !== "active") throw new HttpError(401, "DRIVE_CONNECTION_UNAVAILABLE");
  if (credentials.token_expires_at && new Date(credentials.token_expires_at).getTime() > Date.now() + 60_000) return decrypt(credentials.access_token_encrypted);
  ...
  if (!response.ok || !token.access_token) { await admin.from("drive_connections").update({ status: "expired" }).eq("id", connectionId); throw new HttpError(401, "DRIVE_RECONNECT_REQUIRED"); }
  const expiresAt = new Date(Date.now() + Number(token.expires_in || 3600) * 1000).toISOString();
  await Promise.all([
    admin.schema("private").from("drive_credentials").update({ access_token_encrypted: await encrypt(token.access_token), token_expires_at: expiresAt }).eq("connection_id", connectionId),
